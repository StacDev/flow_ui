...
Values come from the Flow UI Figma file. Role names follow Material 3's `ColorScheme` so a host can map an existing scheme across, with one addition — the design draws content at three ink levels (`onSurface` / `onSurfaceVariant` 75% / `onSurfaceMuted` 50%) and M3 names only two. Two rules hold the palette together: the ink ramp and the outlines are **translucent** (the same label and hairline read correctly on the page and on a raised card), while every `surface*` token is **opaque** (they get scrimmed and drawn over host images). The container ladder `Lowest → Highest` is the design's even ink tints — 2% / 4% / 6% / 8% / 10% — flattened over each theme's page surface so the tokens stay opaque; the raised card — the composer, menus, sheets — sits on `surfaceBright` (white / `#1E1E1E`), the one surface outside the ladder that lifts off the page in both themes.
