...
- `enabled` — `false` holds the line still in the base ink, as does the
  platform's reduced-motion setting.
- `baseColor` (`onSurfaceMuted`), `highlightColor` (`onSurface`),
  `style`, `duration`, `textAlign` — retheme the sweep without touching
  the global tokens.
