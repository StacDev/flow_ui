...
`selectable: false` leaves selection to the host, for a `SelectionArea`
around a larger surface or a long-press the host needs for something
else. A `FlowMessage`, `FlowMarkdown` or `FlowStreamingText` on its own
