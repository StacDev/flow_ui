...
and stays inert rather than opening an empty preview.
...
- `FlowAttachment` — `id`, `thumbnail` (any `ImageProvider`), `kind`,
  `label`.
- `FlowAttachmentGroup` — `attachments`, `layout`
  (`FlowAttachmentLayout.scroll` default / `.wrap`), `onRemove`,
  `onTap`, and the host-supplied tooltips.
- `showFlowAttachmentPreview` — the full-screen viewer, callable directly
  when you override `onTap` or `onAttachmentTap`.
