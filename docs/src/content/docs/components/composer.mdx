...
required — a composer needs somewhere to send to.
...
- `onSend` — required; receives the draft text.
...
- `attachments`, `onRemoveAttachment`, `removeAttachmentTooltip` — the
  pending row.
- `controller`, `placeholder` — the field itself; pass your own
  `TextEditingController` to prefill or clear the draft.
