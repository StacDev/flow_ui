...
`FlowMenuStyle` overrides the menu's metrics and colors per instance —
corner radius, paddings, inks — without touching the global tokens.
...
- `icon`, `tooltip` — the trigger.
- `sheetTitle`, `presentation` — sheet form and when it's used.
- `style` — a `FlowMenuStyle` for per-instance retheming.
