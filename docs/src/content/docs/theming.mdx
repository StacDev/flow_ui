...
ink in light, 10% in dark) and `outlineVariant` (the firm one — 12% and
16%) are the foreground ink at an alpha, not resolved colors — the same
label and the same hairline read correctly on the page *and* on a raised
card, because they composite. The `surfaceContainerLowest → Highest`
